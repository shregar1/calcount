...
from dtos.requests.user.company.register import RegisterCompanyRequestDTO
...
from services.user.company.register import CompanyRegistrationService
...
class UserCompanyRegisterstionController(IController):

    def __init__(self, urn: str = None) -> None:
        super().__init__(urn)
        self.api_name = APILK.COMPANY_REGISTRATION

    async def post(
        self, request: Request, request_payload: RegisterCompanyRequestDTO
    ):
        ...
        try:
            ...
            self.request_payload.update({"user_type": UserType.COMPANY})
            ...
            response_dto: BaseResponseDTO = BaseResponseDTO(
                transactionUrn=self.urn,
                status=APIStatus.FAILED,
                responseMessage="Failed to register company.",
