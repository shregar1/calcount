...
from dtos.requests.user.interviewer.register import (
    RegisterInterviewerRequestDTO,
)
...
from services.user.interviewer.register import (
    UserInterviewerRegistrationService,
)
...
class UserInterviewerRegisterstionController(IController):

    def __init__(self, urn: str = None) -> None:
        super().__init__(urn)
        self.api_name = APILK.INTERVIEWER_REGISTRATION

    async def post(
        self, request: Request, request_payload: RegisterInterviewerRequestDTO
    ):
        ...
        try:
            ...
            self.request_payload.update({"user_type": UserType.INTERVIEWER})

            self.logger.debug("Running user registration service")
            service = UserInterviewerRegistrationService(
                urn=self.urn, user_urn=self.user_urn, api_name=self.api_name
            )
            response_payload: dict = await service.run(
