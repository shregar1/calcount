...
from dtos.requests.user.employer.register import RegisterEmployerRequestDTO
...
from services.user.employer.register import UserEmployerRegistrationService
...
class UserEmployerRegisterstionController(IController):

    def __init__(self, urn: str = None) -> None:
        super().__init__(urn)
        self.api_name = APILK.EMPLOYER_REGISTRATION

    async def post(
        self, request: Request, request_payload: RegisterEmployerRequestDTO
    ):
        ...
        try:
            ...
            self.request_payload.update({"user_type": UserType.EMPLOYER})

            self.logger.debug("Running user registration service")
            service = UserEmployerRegistrationService(
                urn=self.urn, user_urn=self.user_urn, api_name=self.api_name
            )
            response_dto: dict = await service.run(data=self.request_payload)
